...
class _Semaphore {
  ...
  set maxValue(int value) => _max = value;
...
class ReciterStoreService {
  ...
  /// Speed detection & adaptive settings
  Duration _downloadStart = Duration.zero;
  int _downloadedBytes = 0;
  double _currentDownloadSpeed = 0.0;
  int _speedSurahsCompleted = 0;
  static const int _warmupSurahs = 5;
  bool _settingsAdjusted = false;

  /// Adaptive download settings, adjusted by speed detection after warmup.
  int _maxDownloadConnections = 16;
  int _maxChunks = 8;

  /// Files at least this large are downloaded via parallel range requests.
  static const int _chunkedDownloadMinSize = 4 * 1024 * 1024;

  final _Semaphore _connectionLimiter = _Semaphore(16);
  ...
  bool isPaused(String reciterId) => _paused.contains(reciterId);

  /// Pause a download: cancel the token but keep downloaded files so resume
  /// can pick up where it left off.
  void pauseDownload(String reciterId) {
    if (!_downloading.contains(reciterId)) return;
    _paused.add(reciterId);
    final token = _cancelTokens.remove(reciterId);
    if (token != null && !token.isCancelled) {
      token.cancel();
    }
    _emitStatus(reciterId, 'Download paused');
  }

  /// Resume a paused download by re-triggering the download flow.
  /// Existing surah files on disk are kept and reused.
  Future<void> resumeDownload(String reciterId) async {
    _paused.remove(reciterId);
  ...
  Future<void> cancelDownload(String reciterId) async {
    ...
    _currentProgress.remove(reciterId);
    HiveService.clearDownloadCheckpoint(reciterId);
  ...
  int _chunkCountForSize(int size) {
    if (size >= 64 * 1024 * 1024) return _maxChunks;
    if (size >= 16 * 1024 * 1024) return math.min(_maxChunks, 6);
    return math.min(_maxChunks, 4);
  }
  ...
  /// Detects download speed after [ _warmupSurahs ] surahs and applies
  /// optimal connection, chunk, and worker settings for the remaining surahs.
  int _detectAndReportSpeed() {
    if (_settingsAdjusted || _downloadedBytes < 1024) {
      return 10;
    }
    if (_downloadStart == Duration.zero) {
      return 10;
    }

    final elapsed = _downloadStart.inMilliseconds / 1000.0;
    if (elapsed < 2.0) return 10; // need minimum 2 seconds of data

    final speed = _downloadedBytes / elapsed; // bytes/sec
    _currentDownloadSpeed = speed;

    int workers;

    if (speed < 50 * 1024 * 1024) {
      // < 50 MB/s -> conservative
      _maxDownloadConnections = 8;
      _maxChunks = 4;
      workers = 8;
    } else if (speed < 150 * 1024 * 1024) {
      // 50-150 MB/s -> moderate
      _maxDownloadConnections = 16;
      _maxChunks = 6;
      workers = 10;
    } else if (speed < 300 * 1024 * 1024) {
      // 150-300 MB/s -> aggressive
      _maxDownloadConnections = 32;
      _maxChunks = 8;
      workers = 12;
    } else {
      // 300+ MB/s -> maximum
      _maxDownloadConnections = 48;
      _maxChunks = 12;
      workers = 15;
    }

    _settingsAdjusted = true;
    _connectionLimiter.maxValue = _maxDownloadConnections;

    _emitStatus(
      '',
      'Speed: ${(speed / 1024 / 1024).toStringAsFixed(1)} MB/s '
          '– $_maxDownloadConnections connections, '
          '$_maxChunks chunks, $workers workers',
    );
  ...
  Future<void> downloadReciter(ReciterModel reciter) async {
    debugPrint('Starting download of reciter ${reciter.id}');
    ...
    _downloadStart = Duration(milliseconds: DateTime.now().millisecondsSinceEpoch);
    _downloadedBytes = 0;
    _settingsAdjusted = false;
    _speedSurahsCompleted = 0;
    // --------------------------------
    ...
    try {
      ...
      // Restore checkpoint: surahs that were fully downloaded before the
      // process was killed.  They skip the network entirely so the user
      // doesn't re-download data they already paid for.
      final checkpointedSurahs = HiveService.getDownloadCheckpoint(reciter.id);
      final checkpointSet = <int>{...checkpointedSurahs};
      for (final s in checkpointedSurahs) {
        if (s >= 1 && s <= surahCount) {
          final surahKey = s.toString().padLeft(3, '0');
          final f = File('$dir/$surahKey.opus');
          if (await f.exists() && (await f.length()) > 1024) {
            completedBytes += await f.length();
            completedSurahs++;
          } else {
            checkpointSet.remove(s);
          }
        }
      }
      ...
      Future<void> downloadSurah(int surah) async {
        if (checkpointSet.contains(surah)) {
          emitProgress();
          return;
        }
        ...
        if (downloaded) {
          ...
          // Persist checkpoint so a killed process can resume here.
          checkpointSet.add(surah);
          HiveService.saveDownloadCheckpoint(
            reciter.id,
            checkpointSet.toList()..sort(),
          );

          // --- Speed detection: track bytes and surah count ---
          _downloadedBytes += await file.length();
          _speedSurahsCompleted++;
          if (_speedSurahsCompleted >= _warmupSurahs && !_settingsAdjusted) {
            _detectAndReportSpeed();
          }
          // --------------------------------------------------
        }
      ...
      if (actualCount >= surahCount) {
        ...
        HiveService.clearDownloadCheckpoint(reciter.id);
      } else {
        _emitStatus(
          reciter.id,
